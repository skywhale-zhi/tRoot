...
{
    //神圣链球
    internal class HallowedMaceProjectile : ModProjectile
    {
        private const string ChainTexturePath = "tRoot/Content/Projectiles/Warrior/HallowedMaceProjectileChain";
        private const string ChainTextureExtraPath = "tRoot/Content/Projectiles/Warrior/HallowedMaceProjectileChainExtra";
        ...
        private bool RightMode = false; //右键模式，记录伤害减损
        ...
        public override void AI()
        {
            ...
            int launchTimeLimit = 17;  //最大发射时间：弹丸在缩回之前可以移动多少时间（速度和shootTimer将设置连枷的射程）
            float launchSpeed = 18f; // 弹丸的发射速度
            float maxLaunchLength = 1000f; // 最大发射距离：在发射状态下，弹丸链在强制收回之前可以伸展多远
            ...
            float maxForcedRetractSpeed = 30f; // 弹丸在被迫收回时的最大速度
            ...
            int ricochetTimeLimit = launchTimeLimit - 5; //跳弹时限
            ...
            float maxDroppedRange = launchRange + 126f;     //最大牵拉距离
                ...
                case AIState.Spinning://旋转
                    {
                        ...
                        {   //朝向鼠标的单位矢量
                            ...
                            if (player.controlUseItem && !Main.mouseRight)
                                RightMode = false;
                            if(RightMode && SpinningStateTimer >= 20)
                            {
                                RightMode = true;
...
//                          if (!player.channel) // 如果玩家释放，则切换到发射模式(原版)
                            if (!player.channel && !Main.mouseRight && !RightMode) // 右键版，上面是原版
                            {
                                RightMode = false;
                                CurrentAIState = AIState.LaunchingForward;
                                StateTimer = 0f;
            ...
            if (Main.rand.NextBool(dustRate))
            {
                for(int i = 0; i < 2; i++)
                {
                    Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.Enchanted_Gold, 0f, 0f, 50, default(Color), 1.25f);
                    dust.noGravity = true;
                }
            }
        }
        ...
        {
            // Flails do special collision logic that serves to hit anything within an ellipse centered on the player when the flail is spinning around the player.
            // For example, the projectile rotating around the player won't actually hit a bee if it is directly on the player usually, but this code ensures that the bee is hit.
            // This code makes hitting enemies while spinning more consistant and not reliant of the actual position of the flail projectile.
            ...
            if (CurrentAIState == AIState.Spinning)
            {
                ...
                shortestVectorFromPlayerToTarget.Y /= 0.8f; //使命中区域成为椭圆。由于这个数学模型，垂直命中距离更小。
                float hitRadius = 55f; // 椭圆半长半径的长度（长端）
